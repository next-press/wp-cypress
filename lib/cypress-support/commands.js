...
const commands = {
  ...
  create(fixture, options) {
    cy.exec(`node_modules/.bin/wp-cypress wp "fixture ${fixture} '${JSON.stringify(options).replace(/"/g, '\\"')}'"`).then((result) => {
      cy.log(result.stdout);
    });
  },

  update(fixture, options) {
    cy.exec(`node_modules/.bin/wp-cypress wp "fixture ${fixture} '${JSON.stringify(options).replace(/"/g, '\\"')}' --update"`).then((result) => {
      cy.log(result.stdout);
    });
  },
