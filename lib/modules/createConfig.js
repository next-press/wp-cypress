...
const getThemeAndPluginDataFromDir = require('./getThemeAndPluginDataFromDir');
const getCustomVolumes = require('./getCustomVolumes');
...
const createConfig = async (packageDir, hasVolumeSupport = true) => {
  ...
  const config = defaults(wp, {
    ...
    fixturesPath: 'cypress/fixtures/wp',
    wpContent: false,
    multisite: false,
    url: false,
  });
  ...
  if (config.wpContent) {
    ...
    const { pluginsFromDir, themesFromDir } = getThemeAndPluginDataFromDir(config.pluginsDir, config.themesDir);
    const { customVolumes } = getCustomVolumes(config.volumes);
    ...
    volumes = [
      ...volumes,
      ...(customVolumes.map((x) => x.volume)),
      ...(plugins.map((x) => x.volume)),
      ...(themes.map((x) => x.volume)),
      ...(pluginsFromDir.map((x) => x.volume)),
      ...(themesFromDir.map((x) => x.volume)),
    ];
  }

  const seedsDir = `${CWD}/${config.seedsPath}`;
  const fixturesDir = `${CWD}/${config.fixturesPath}`;

  createDefaultSeeds(seedsDir, packageDir);

  volumes.push(
    `${seedsDir}:/var/www/html/seeds`,
    `${fixturesDir}:/var/www/html/fixtures`,
